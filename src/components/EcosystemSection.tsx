...
const EcosystemCard = ({ ecosystem, index }: { ecosystem: typeof ecosystems[0]; index: number }) => {
  return (
    <motion.div
...
const EcosystemSection = () => {
  return (
    <section id="ecosystem" className="py-20 relative overflow-hidden cosmic-grid">
      <div className="container mx-auto px-4">
        ...
        >
          <h2 className="text-4xl font-bold mb-4 text-alien-gold text-glow">Unified Ecosystem</h2>
          <p className="text-xl text-gray-300 max-w-3xl mx-auto">
            AlienFlowSpace bridges diverse decentralized domains into a coherent, interoperable ecosystem.
          </p>
        </motion.div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6">
          {ecosystems.map((ecosystem, index) => (
            <EcosystemCard key={ecosystem.id} ecosystem={ecosystem} index={index} />
          ))}
        </div>
      </div>
